...
struct ElasticRod2::MinimizationPImpl
{
public:
    MinimizationPImpl() { }
    ~MinimizationPImpl() { }

    double minimize(const ElasticRod2 *rod, ColumnVector& io_theta);

public:

    static void extractThetaVars(const ElasticRod2* rod, const ColumnVector& theta, ColumnVector& o_thetaVars);
    static void constructTheta(const ElasticRod2* rod, const ColumnVector& thetaVars, ColumnVector& o_theta);
...
//====================================== ElasticRod2 implementation =======================================

ElasticRod2::ElasticRod2(const ElasticRodParams *params)
{
    assert( params != NULL );
    m_params = params;
...
ElasticRod2::~ElasticRod2()
{
    delete m_minimization;
}

void ElasticRod2::init(const std::vector<mg::Vec3D>& restpos,
                      const mg::Vec3D& u0,
...
void ElasticRod2::getState(ElasticRodState& o_state) const
{
...
void ElasticRod2::setState(const ElasticRodState& state)
{
    m_ppos = state.m_ppos;
    m_pvel = state.m_pvel;
    m_u0 = state.m_u0;
...
void ElasticRod2::computeEdges(const std::vector<mg::Vec3D>& vertices,
...
void ElasticRod2::computeLengths(const std::vector<mg::Vec3D>& edges,
...
void ElasticRod2::computeMaterialCurvature(const std::vector<mg::Vec3D>& kb,
...
void ElasticRod2::computeW(const mg::Vec3D& kb, const mg::Vec3D& m1, const mg::Vec3D& m2, mg::Vec2D& o_wij) const
{
    o_wij.set( mg::dot( kb, m2 ), -mg::dot( kb, m1 ));
}

void ElasticRod2::computeKB(const std::vector<mg::Vec3D>& edges,
...
void ElasticRod2::extractSinAndCos(const double &magnitude,
...
void ElasticRod2::computeBishopFrame(const mg::Vec3D& u0,
...
void ElasticRod2::parallelTransportFrame(const mg::Vec3D& e0, const mg::Vec3D& e1,
                                        mg::Vec3D& io_u) const
{
...
void ElasticRod2::computeMaterialFrame(const ColumnVector &theta,
...
void ElasticRod2::applyInternalConstraintsIteration()
{
    mg::Vec3D e;
    mg::Real l, l1, l2;
...
void ElasticRod2::accumulateInternalElasticForces(std::vector<mg::Vec3D>& o_forces)
{
...
void ElasticRod2::computeGradientKB(const std::vector<mg::Vec3D> &kb,
                                   const std::vector<mg::Vec3D> &edges,
...
void ElasticRod2::computeGradientHolonomyTerms(const std::vector<mg::Vec3D> &kb,
...
void ElasticRod2::computeGradientCurvature(unsigned i, unsigned k, unsigned j,
...
void ElasticRod2::computeGradientHolonomy(unsigned i , unsigned j,
                                 const std::vector<mg::Vec3D>& minusGH,
                                 const std::vector<mg::Vec3D>& plusGH,
                                 const std::vector<mg::Vec3D>& eqGH,
                                 mg::Vec3D& o_GH) const
...
void ElasticRod2::computeEnergy(const std::vector<mg::Vec3D>& m1,
                               const std::vector<mg::Vec3D>& m2,
                               const ColumnVector &theta,
                               mg::Real &o_E) const
{
...
void ElasticRod2::computedEdQj(unsigned j,
                              const mg::Vec3D& m1j,
                              const mg::Vec3D& m2j,
                              const ColumnVector &theta,
...
void ElasticRod2::computeHessian(unsigned j,
                                const mg::Vec3D& m1j,
                                const mg::Vec3D& m2j,
                                const mg::Matrix2D& J,
                                mg::Real &o_Hjjm1,
...
void ElasticRod2::updateCurrentState()
{
...
void ElasticRod2::MinimizationPImpl::extractThetaVars(const ElasticRod2 *rod, const ColumnVector& theta, ColumnVector& o_thetaVars)
{
...
void ElasticRod2::MinimizationPImpl::constructTheta(const ElasticRod2 *rod, const ColumnVector& thetaVars, ColumnVector& o_theta)
{
...
double ElasticRod2::MinimizationPImpl::minimize(const ElasticRod2* rod, ColumnVector &io_theta)
{
...
double ElasticRod2::MinimizationPImpl::evaluate::operator ()(const ColumnVector& theta) const
{
...
ColumnVector ElasticRod2::MinimizationPImpl::evaluateGradient::operator ()(const ColumnVector &theta) const
{
...
Hessian ElasticRod2::MinimizationPImpl::evaluateHessian::operator ()(const ColumnVector& theta) const
{
