...
public:
	...
	{
		auto it = std::find_if( s.begin(), s.end(), std::ptr_fun<int, int>( std::isspace ) );
		return s.substr( 0, it - s.begin() );
	}

	static inline std::string parseWord( const std::string& s )
	{
		auto it = std::find_if( s.begin(), s.end(), std::not1( std::ptr_fun<int, int>( std::isalpha ) ) );
		return s.substr( 0, it - s.begin() );
	}
	...
	{
		char* end;
		o_v[0] = static_cast<mg::Real>( std::strtod( s.c_str(), &end ) );
		o_v[1] = static_cast<mg::Real>( std::strtod( end, &end ) );
		o_v[2] = static_cast<mg::Real>( std::strtod( end, &end ) );
	}
	...
	{
		char* end;
		auto val_x = static_cast<mg::Real>( std::strtod( s.c_str(), &end ) );
		auto val_y = static_cast<mg::Real>( std::strtod( end, &end ) );
		auto val_z = static_cast<mg::Real>( std::strtod( end, &end ) );
		mg::matrix_rotation_euler( o_m,
		                           mg::rad( val_x ),
		                           mg::rad( val_y ),
		                           mg::rad( val_z ),
		                           mg::euler_order_xyz );

		val_x = static_cast<mg::Real>( std::strtod( end, &end ) );
		val_y = static_cast<mg::Real>( std::strtod( end, &end ) );
		val_z = static_cast<mg::Real>( std::strtod( end, &end ) );
		mg::Matrix4D scale;
	...
	static inline void parseIntList( std::string& s, std::vector<unsigned>& o_list )
	{
		...
		char* end;
		for( auto i = 0u; i < o_list.size(); ++i )
		{
			o_list[i] = static_cast<unsigned>( std::strtod( start, &end ) );
			start = end;
		}
	}
};
